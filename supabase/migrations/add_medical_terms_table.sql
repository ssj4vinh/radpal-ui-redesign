-- Create table for storing user's custom medical terms with weights
CREATE TABLE IF NOT EXISTS medical_terms (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  term TEXT NOT NULL,
  weight INTEGER NOT NULL CHECK (weight >= -5 AND weight <= 5),
  category TEXT NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
  UNIQUE(user_id, term, category)
);

-- Create index for faster lookups
CREATE INDEX idx_medical_terms_user_id ON medical_terms(user_id);
...
-- Create RLS policies
CREATE POLICY "Users can view their own medical terms" ON medical_terms
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own medical terms" ON medical_terms
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own medical terms" ON medical_terms
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own medical terms" ON medical_terms
  FOR DELETE USING (auth.uid() = user_id);

-- Create function to upsert medical terms
...
-- Create function to bulk upsert medical terms
...
-- Create function to get all user's medical terms
...
-- Insert default medical terms for new users (will be called from app)
