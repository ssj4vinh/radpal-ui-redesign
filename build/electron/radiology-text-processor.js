class RadiologyTextProcessor {
  ...
  processFormattingCommands(text) {
    ...
    processed = processed.replace(/[ \t]+/g, ' ');  // Replace multiple spaces/tabs with single space (but preserve newlines)
    ...
    // Handle line breaks and paragraphs - do the replacement directly (not just marking)
    // Handle all variations of paragraph and new line commands
  ...
  processText(text) {
    ...
    // This preserves intentional line breaks that are the entire content
    if (processed.replace(/\s/g, '').length > 0) {
      // There's actual content, safe to trim edges
