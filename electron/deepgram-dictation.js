...
class DeepgramDictationManager {
  ...
  loadCustomKeywords() {
    try {
      ...
      if (fs.existsSync(keywordsPath)) {
        ...
        // Flatten all keyword categories into a single array
        for (const category of Object.values(keywordsData.keywords)) {
          if (typeof category === 'object' && !Array.isArray(category)) {
            for (const [word, weight] of Object.entries(category)) {
              if (word !== '_comment') {
                allKeywords.push(`${word}:${weight}`);
              }
            }
          }
        }
        
        this.customKeywords = allKeywords;
        console.log(`📚 Loaded ${allKeywords.length} medical keywords for transcription boosting`);
      }
    } catch (error) {
  ...
  async initializeDeepgram() {
    try {
      ...
      // Use custom keywords if loaded, otherwise use default set
      let medicalKeywords;
      
      if (this.customKeywords && this.customKeywords.length > 0) {
        // Use loaded keywords from JSON file
        medicalKeywords = this.customKeywords.join(',');
        console.log(`🎯 Using ${this.customKeywords.length} custom medical keywords`);
      } else {
        // Fallback to default keywords if file not found
        console.log('📝 Using default medical keywords');
        medicalKeywords = [
          'chondrolabral:2',
          'meniscal:2',
          'menisci:2',
      ...
      // Deepgram API endpoint for real-time transcription with Nova-2 model
      // Nova-2 is the standard model that works with most API keys
      const deepgramUrl = `wss://api.deepgram.com/v1/listen?` + 
        `encoding=linear16&` +
        `sample_rate=16000&` +
        `language=en-US&` +
        `model=nova-2&` +
        `punctuate=false&` +           // We handle punctuation commands manually
        `smart_format=false&` +         // We handle formatting ourselves
        `diarize=false&` +              // Single speaker
        `multichannel=false&` +         // Single channel
        `endpointing=200&` +            // Reduced from 300ms for faster response
        `interim_results=true&` +       // Show partial results for responsiveness
        `utterance_end_ms=1000&` +     // Faster utterance detection
        `vad_events=true&` +            // Voice activity detection
        `numerals=true&` +              // Convert number words to digits
        `keywords=${encodeURIComponent(medicalKeywords)}`  // Boost medical terms
        ...
        try {
          ...
          if (response.type === 'Results') {
            ...
            if (transcript && transcript.trim().length > 0) {
              ...
              const isFinal = response.is_final || response.channel?.alternatives?.[0]?.is_final;
              ...
              if (isFinal) {
                ...
                // IMPORTANT: Replace periods that Deepgram adds from pauses with spaces
                ...
                cleanedTranscript = cleanedTranscript.replace(/\./g, ' ').replace(/\s+/g, ' ').trim();
