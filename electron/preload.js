...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  onTriggerReportGeneration: (callback) => {
    const handler = () => callback();
    ipcRenderer.on('trigger-report-generation', handler);
    return () => ipcRenderer.removeListener('trigger-report-generation', handler);
  },
  onTriggerImpressionGeneration: (callback) => {
    const handler = () => callback();
    ipcRenderer.on('trigger-impression-generation', handler);
    return () => ipcRenderer.removeListener('trigger-impression-generation', handler);
  },
  ...
  // Macro IPC methods
  listMacros: (userId, scope) => ipcRenderer.invoke('list-macros', userId, scope),
  getMacro: (userId, name) => ipcRenderer.invoke('get-macro', userId, name),
  saveMacro: (macro) => ipcRenderer.invoke('save-macro', macro),
  deleteMacro: (userId, macroId) => ipcRenderer.invoke('delete-macro', userId, macroId),
  
  // Login panel IPC methods
  checkInviteCode: (inviteCode) => ipcRenderer.invoke('check-invite-code', inviteCode),
  markInviteCodeUsed: (inviteCode, userId, email, firstName, lastName) => 
    ipcRenderer.invoke('mark-invite-code-used', { inviteCode, userId, email, firstName, lastName }),
  ...
  closeAppGentle: () => ipcRenderer.send('close-app-gentle'),
  ...
  // Dictation import hotkey compilation
  compileDictationImportHotkeys: (config) => ipcRenderer.invoke('compile-dictation-import-hotkeys', config),
  
  // Global impression base prompt methods
  fetchGlobalImpressionPrompt: () => ipcRenderer.invoke('fetch-global-impression-prompt'),
  updateGlobalImpressionPrompt: (userId, newPrompt) => ipcRenderer.invoke('update-global-impression-prompt', userId, newPrompt),
  
  // Global prompt sections methods
  fetchGlobalPromptSections: () => ipcRenderer.invoke('fetch-global-prompt-sections'),
  updateGlobalFindingsRules: (userId, rules) => ipcRenderer.invoke('update-global-findings-rules', userId, rules),
  updateGlobalImpressionRules: (userId, rules) => ipcRenderer.invoke('update-global-impression-rules', userId, rules),
  
  // Medical terms methods
  fetchMedicalTerms: () => ipcRenderer.invoke('fetch-medical-terms'),
  saveMedicalTerms: (termsArray) => ipcRenderer.invoke('save-medical-terms', termsArray),
  addMedicalTerm: (term, weight, category) => ipcRenderer.invoke('add-medical-term', term, weight, category),
  deleteMedicalTerm: (term, category) => ipcRenderer.invoke('delete-medical-term', term, category),
  loadMedicalKeywordsFromDB: () => ipcRenderer.invoke('load-medical-keywords-from-db')
});
