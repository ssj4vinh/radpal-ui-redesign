...
export function useAgentReport() {
  ...
  const generateReportWithAgent = async (
    ...
    studyType: string,
    model?: string
  ): Promise<{ text: string; tokens: { input: number; output: number; total: number }; prompt?: string }> => {
    if (!user?.id) {
    ...
    try {
      ...
      return {
        ...
        prompt: reportResponse.prompt // Include the prompt for display
      }
    } catch (err) {
  ...
  const generateImpressionWithAgent = async (
    ...
    try {
      ...
      return {
        ...
        prompt: impressionResponse.prompt // Include the prompt for display
      }
    } catch (err) {
