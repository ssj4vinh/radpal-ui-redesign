      ...
      return (
        <div className="window-frame">
          ...
          <TemplateManager
            ...
            onSave={async (studyType, t, p, i) => {
              console.log('💾 Attempting to save:', studyType)
              try {
                await saveTemplate(studyType, t, p, i)
                showNotification('✓ Saved!', 'success')
                // Refresh templates to get the latest data including keywords
            ...
            onDelete={async (studyType) => {
              console.log('🗑️ Attempting to delete:', studyType)
              try {
                const result = await window.electron?.ipcRenderer?.invoke('delete-template', {
                  userId: user?.id,
                  studyType
                })
                if (result?.success) {
                  // Refresh templates
                  await refetchTemplates()
                  showNotification(`✓ Deleted "${studyType}"`, 'success')
                } else {
            ...
            onSaveWithAgentLogic={async (studyType, template, agentLogic, showDiffView, keywords) => {
  ...
  .catch((err) => {
    ...
    if (mode === 'gpt-diff') {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        {/* Diff display - switches between textarea and diff view */}
        {isEditingDiff ? (
          <div style={{ position: 'relative' }}>
            <div style={{
              ...
              Press ESC to view diff • Click outside to keep editing
            </div>
          <textarea
            ...
            onKeyDown={(e) => {
              // Only exit edit mode on Escape, not on Enter
              if (e.key === 'Escape') {
            ...
            <div style={{ display: 'inline', whiteSpace: 'pre-wrap' }}>
              {isRefining ? (
                ...
                computedDiff?.map((part, i) => (
                  <span key={`diff-${i}-${part.added ? 'add' : part.removed ? 'rem' : 'same'}`} style={{...getPartStyle(part), whiteSpace: 'pre-wrap'}}>
                    {part.value}
                  </span>
