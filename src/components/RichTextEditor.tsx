  ...
  ({ value, onChange, placeholder, style, className, theme }, ref) => {
    ...
    const handleInput = useCallback(() => {
      if (editorRef.current && !isInternalChange.current) {
        ...
        // Save cursor position before onChange
    ...
    const insertDictationUsingMarkers = useCallback(async (raw: string) => {
      ...
      // Line breaks are already converted to \n or \n\n by insertDictation
      // Only apply smart spacing for regular text (not line breaks)
      if (insertText !== '\n' && insertText !== '\n\n') {
      ...
      try {
        if (insertText === '\n' || insertText === '\n\n') {
          // For line breaks, insert BR elements
          if (insertText === '\n') {
            // Single line break
            const br = document.createElement('br');
            caretRange.insertNode(br);
            // Move caret after the BR
            caretRange.setStartAfter(br);
            caretRange.collapse(true);
            insertedNode = br;
          } else {
            // Double line break for paragraph  
            const br1 = document.createElement('br');
            const br2 = document.createElement('br');
            caretRange.insertNode(br1);
            caretRange.setStartAfter(br1);
            caretRange.insertNode(br2);
            caretRange.setStartAfter(br2);
            caretRange.collapse(true);
            insertedNode = br2;
          }
        } else {
    ...
    useImperativeHandle(ref, () => ({
      insertDictation: (text: string) => {
        ...
        // Removed debounce to allow continuous dictation flow
        ...
        if (leadingNewlines) {
          ...
          console.log('🎤 Text has leading newlines:', { 
            newlineCount, 
          ...
          if (selection && selection.rangeCount > 0) {
            ...
            if (textAfterNewlines) {
              let processedText = convertNumberWords(textAfterNewlines.trim())
          ...
          console.log('🎤 Empty text received, skipping')
          return
        }
        
        // For regular text, just use simple insertion
        // Convert number words to digits
      ...
      getValue: () => {
        let value = editorRef.current?.innerHTML || lastValue.current || ''
        // Clean zero-width spaces from the HTML
        value = value
    ...
    }), [onChange, formatText, convertNumberWords, captureSelectionIfInEditor, flushPendingExternal])
