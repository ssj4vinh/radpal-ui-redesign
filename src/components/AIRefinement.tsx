...
import { X, Copy, Check, RefreshCw, ChevronLeft, ChevronRight, Maximize2, Minimize2, Settings, Eye, EyeOff, Edit3 } from 'lucide-react'
...
export default function AIRefinement({
  ...
  const handleConservativeModeChange = (checked: boolean) => {
    ...
    // Reset custom prompt when switching modes
  ...
  const generateRefinement = async () => {
    ...
    try {
      ...
      let refinementPrompt = conservativeMode ? conservativeRefinementPrompt : standardRefinementPrompt
      
      // Use custom prompt if available and enabled
      if (useCustomPrompt && editablePrompt) {
        refinementPrompt = editablePrompt
      }
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <button
              onClick={() => {
                ...
                if (!editablePrompt) {
                  setEditablePrompt(conservativeMode ? conservativeRefinementPrompt : standardRefinementPrompt)
                }
              }}
              style={{
                background: showPromptEditor ? 'rgba(59, 130, 246, 0.2)' : 'rgba(255, 255, 255, 0.05)',
                border: 'none',
                borderRadius: '6px',
                padding: '6px 10px',
                cursor: 'pointer',
                color: showPromptEditor ? '#3B82F6' : '#ffffff',
                display: 'flex',
                alignItems: 'center',
                gap: '6px',
                fontSize: '12px'
              }}
              title={showPromptEditor ? 'Hide prompt editor' : 'Show/edit prompt'}
            >
              {showPromptEditor ? <EyeOff size={14} /> : <Eye size={14} />}
              Prompt
            </button>
            <label style={{ 
              display: 'flex', 
        ...
        {showPromptEditor && (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <span style={{ fontSize: '14px', fontWeight: 500, color: '#ffffff' }}>
                  {conservativeMode ? 'Conservative Mode Prompt' : 'Standard Mode Prompt'}
                </span>
                <label style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '6px',
                  fontSize: '12px',
                  color: useCustomPrompt ? '#3B82F6' : 'rgba(255, 255, 255, 0.6)',
                  cursor: 'pointer'
                }}>
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) => {
                      ...
                      if (!e.target.checked) {
                        // Reset to default prompt when unchecking
                        setEditablePrompt(conservativeMode ? conservativeRefinementPrompt : standardRefinementPrompt)
                      }
                    }}
            ...
            <textarea
              ...
              placeholder="Enter your custom prompt. Use {REPORT_TEXT} as placeholder for the report."
              style={{
                width: '100%',
                minHeight: '150px',
                maxHeight: '300px',
            ...
            <p style={{
              ...
              Tip: Use {'{REPORT_TEXT}'} in your prompt to indicate where the report should be inserted.
            </p>
          </div>
        )}
