...
export interface ConditionalExclusion {
  finding: string;  // The finding to exclude (e.g., "small joint effusion")
  unless?: string;  // Optional condition when to include it (e.g., "there is associated synovitis")
...
interface ImpressionCategory {
  ...
  exclude_by_default?: Array<string | ConditionalExclusion>; // Support both string and conditional exclusions
}
...
interface StudyImpressionCategory {
  required_opening_phrase?: {
    enabled: boolean;
    phrase: string;
    numbered?: boolean;  // Whether to number this opening phrase (default: true)
  };
  priority?: {
...
export function getDefaultStandardizedLogic(): StandardizedAgentLogic {
  return {
    ...
    general: {
      corrections: {
        rules: []  // No hardcoded defaults - should come from database
      },
      tone: {
        style: 'balanced'
      },
  ...
  return {
    ...
    study_impression: {
      required_opening_phrase: {
        enabled: false,
        phrase: "",
        numbered: true  // Default to numbered
      },
      priority: {
