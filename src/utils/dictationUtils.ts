...
export function processVoiceCommands(text: string): VoiceCommandResult {
  ...
  // Check for formatting commands - exact match only
  if (trimmedText.match(/^(new paragraph|paragraph)$/i)) {
    return { type: 'NEW_PARAGRAPH', action: 'newParagraph' };
  }
  
  if (trimmedText.match(/^(new line|line break)$/i)) {
  ...
  // Replace inline formatting commands with actual line breaks
  processedText = processedText.replace(/\b(new paragraph|paragraph)\b/gi, '\n\n');
  processedText = processedText.replace(/\b(new line|line break)\b/gi, '\n');
  ...
  processedText = processedText
    ...
    .replace(VOICE_COMMANDS.PERCENTAGE, '%');
  
  // Convert "by" to "x" when between numbers (e.g., "7 by 5 by 3" -> "7 x 5 x 3")
  // This regex matches "by" when preceded and followed by numbers (with optional spaces)
  processedText = processedText.replace(/(\d+(?:\.\d+)?)\s+by\s+(\d+(?:\.\d+)?)/gi, '$1 x $2');
...
function normalizeBoundarySpacing(
  ...
  if (rightChar && insertLastChar) {
    ...
    // 3. Inserted text ends with period/comma/semicolon AND next char is anything except space or punctuation
    const needsSpace = (isAlphanumeric(insertLastChar) && isAlphanumeric(currentRightChar) && 
                       ...
                       (['.', ',', ';', ':', '!', '?'].includes(insertLastChar) && currentRightChar && currentRightChar !== ' ' && !isPunctuation(currentRightChar));
...
export function insertDictationInTextarea(
  ...
  if (normalizedText !== newValue) {
    ...
    // Instead of replacing the entire value which can cause cursor jumping,
    // use setRangeText to replace the entire content while preserving focus
    textarea.setRangeText(normalizedText, 0, textarea.value.length, 'preserve');
    
    // Now set the correct caret position with adjustment
    const finalCaretPos = Math.min(caretPos + caretOffset, normalizedText.length);
    textarea.setSelectionRange(finalCaretPos, finalCaretPos);
    
    // Force focus to prevent any async operations from stealing it
    textarea.focus();
  }
